...
import { ConnectionError, CONSOLE_COLORS, MySQLOptions, DBConnectStatus } from '../../types';
...
export const callbackConnection = (err: MysqlError, context: Connection, server: string): void => {
    if (err) {
      ...
      console.log(`${CONSOLE_COLORS.BgGreen}DB ${server} is Connected: DB_ID: ${context.threadId}${CONSOLE_COLORS.Reset}`);
...
export const EndConnectionMessage = (server: string) => console.log(`${CONSOLE_COLORS.BgGreen}DB ${server} Connection was closed successfully${CONSOLE_COLORS.Reset}`);