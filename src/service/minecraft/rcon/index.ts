...
import type { RCONQueque } from "../../../types";
/**
...
class RCON {
   ...
   */
  connect(server: string, port: number, password: string): Promise<void> {
  ...
  send(command: string): Promise<string> {
    if (!this.online || !this.authenticated) {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.drain();
        this.events.on(String(id), (result) => {
          if (result instanceof Error) {
            reject(result);
          } else {
            if (result.type !== Packet.type["COMMAND_RES"]) {
              ...
              if (result.payload === Packet.payload["COMMAND_END"]) {
                ...
                this.events.removeAllListeners(String(id));
                resolve(response);
              } else {
